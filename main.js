...
const elementsToTranslate = {
    ...
    'dummy-data-loaded': { // This message will no longer be used as dummy data is removed
    ...
    // Ko-fi section translations
    'title-support-me': {
        'en': 'Support Me',
        'jp': '私を応援してください'
    },
    'text-support-me': {
        'en': 'If you find this backtesting tool useful, consider supporting its development!',
        'jp': 'このバックテストツールが役立つと思われたら、開発をサポートすることを検討してください！'
    },
    'text-ko-fi-link': {
        'en': 'ko-fi.com/hudax18',
        'jp': 'ko-fi.com/hudax18'
    }
};
...
function clearTradeInputs() {
    ...
    tradeSlInput.value = '';
    tradeTpInput.value = '';
    tradePairSelect.dispatchEvent(new Event('change')); // Manually trigger change to update step values (and handle potential NaN)
}
...
function handlePriceInputChange() {
    ...
    const inputs = [tradeEntryInput, tradeSlInput, tradeTpInput];
...
function confirmDeleteTrade(id) {
    ...
    modalContent.classList.add('bg-white', 'p-6', 'rounded-lg', 'shadow-xl', 'text-center', 'max-w-sm', 'mx-auto');
...
function confirmResetData() {
    ...
    const message = document.createElement('p');
    message.textContent = confirmationMessage;
    message.classList.add('mb-5', 'text-lg', 'font-bold', 'text-red-600');
...
function renderPipsSummaryTable(monthlyPips) {
    ...
    if (!pipsSummaryTableBody || !totalOverallPipsElement) return;
    ...
    years.forEach(year => {
        ...
        if (totalPipsThisYear > 0) {
            ...
            totalYearCell.classList.remove('text-green-600', 'text-red-600');
        }
...
async function importTradesFromCSV(event) {
    ...
    reader.onload = async (e) => {
        ...
        for (let i = 1; i < lines.length; i++) {
            ...
            const lotSizeVal = parseFloat(document.getElementById('lot-size').value) || 0.01;
            trade.nominalProfitLoss = parseFloat((trade.pips * lotSizeVal * pipValuePerLot).toFixed(2));
...
// Optional: Add some dummy data for demonstration (Removed addDummyData() call from here)
function addDummyData() {
    // This function is now only defined, not called automatically.
    // If you want dummy data, you would need to call it manually
    // or uncomment the call in the DOMContentLoaded listener.
    const lotSizeInput = document.getElementById('lot-size');
    const lotSizeVal = parseFloat(lotSizeInput.value) || 0.01;
    ...
    trades = dummyTrades.map(trade => {
        ...
        const nominalProfitLoss = pips * lotSizeVal * pipValuePerLot;
...
document.addEventListener('DOMContentLoaded', () => {
    // DOM elements references should be inside here or global if they are static/guaranteed to exist
    const tradePairSelect = document.getElementById('trade-pair');
    ...
    priceInputs.forEach(input => {
        input.addEventListener('input', handlePriceInputChange); // Use 'input' event for real-time formatting
        input.addEventListener('change', handlePriceInputChange); // Also 'change' for when input loses focus
    });
    ...
    // Initial load sequence - ensure all DOM elements are available before calling these functions
    loadTrades(); // Load any existing data

    // --- REMOVED AUTOMATIC DUMMY DATA LOADING ---
    // The line below used to call addDummyData() if trades.length was 0.
    // By commenting it out, dummy data will no longer be loaded automatically.
    // if (trades.length === 0) {
    //     addDummyData();
    // }
    // --- END OF REMOVED CODE ---

    renderTradeTable(); // Render table based on loaded/no data
    calculateAndDisplayStatistics(); // Calculate stats based on loaded/no data
    setLanguage(languageSelect.value); // Ensure language is set on load
    showPage('page-data-entry'); // Show the default page

    // Trigger initial handlePriceInputChange for current values on load
